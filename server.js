...
subfolders.forEach(subfolder => {
    const templateSubfolderPath = path.join(templatePath, subfolder);
    const projectSubfolderPath = path.join(projectPath, subfolder);

    if (fs.existsSync(templateSubfolderPath)) {
        console.log(`Copying ${subfolder}...`);
        
        // ✅ COPY ENTIRE FOLDER (including .inp, .py, .f files)
        copyFolderRecursiveSync(templateSubfolderPath, projectSubfolderPath);
        
        // ✅ VERIFY CRITICAL FILES WERE COPIED
        const inpFiles = fs.readdirSync(projectSubfolderPath).filter(f => f.endsWith('.inp'));
        const pyFiles = fs.readdirSync(projectSubfolderPath).filter(f => f.endsWith('.py'));
        const fFiles = fs.readdirSync(projectSubfolderPath).filter(f => f.endsWith('.f'));
        
        console.log(`   ✓ ${subfolder}: ${inpFiles.length} .inp, ${pyFiles.length} .py, ${fFiles.length} .f files`);
        
        if (inpFiles.length === 0) {
            console.warn(`   ⚠️ WARNING: No .inp files found in ${subfolder}`);
        }
        
        // ✅ FOR CDTIRE: Explicitly verify Python and Fortran files
        if (protocol === 'CDTire') {
            const requiredPyFiles = ['deflection.py', 'od_growth.py', 'extract_element_sets.py'];
            const requiredFFiles = ['freeroll.f'];
            
            requiredPyFiles.forEach(pyFile => {
                const pyPath = path.join(projectSubfolderPath, pyFile);
                if (!fs.existsSync(pyPath)) {
                    console.warn(`   ⚠️ WARNING: Missing Python file: ${pyFile} in ${subfolder}`);
                    
                    // ✅ ATTEMPT TO COPY FROM ROOT CDTire TEMPLATE
                    const rootPyPath = path.join(templatePath, pyFile);
                    if (fs.existsSync(rootPyPath)) {
                        fs.copyFileSync(rootPyPath, pyPath);
                        console.log(`   ✓ Copied ${pyFile} from CDTire root to ${subfolder}`);
                    }
                }
            });
            
            requiredFFiles.forEach(fFile => {
                const fPath = path.join(projectSubfolderPath, fFile);
                if (!fs.existsSync(fPath)) {
                    console.warn(`   ⚠️ WARNING: Missing Fortran file: ${fFile} in ${subfolder}`);
                    
                    // ✅ ATTEMPT TO COPY FROM ROOT CDTire TEMPLATE
                    const rootFPath = path.join(templatePath, fFile);
                    if (fs.existsSync(rootFPath)) {
                        fs.copyFileSync(rootFPath, fPath);
                        console.log(`   ✓ Copied ${fFile} from CDTire root to ${subfolder}`);
                    }
                }
            });
        }
        ...
        console.error(`Template subfolder not found: ${templateSubfolderPath}`);
    }
});
...
async function generateBatchFile(projectName, protocol) {
    try {
        ...
        let query;
        if (protocol === 'CDTire') {
            query = `SELECT number_of_runs, test_name, job, old_job, p, l, fortran_file, python_script 
                     FROM ${tableName} 
                     ORDER BY number_of_runs`;
        } else {
            query = `SELECT number_of_runs, tests, job, old_job, p, l 
                     FROM ${tableName} 
                     ORDER BY number_of_runs`;
        }
        ...
        if (!testData || testData.length === 0) {
            throw new Error('No test data found for batch file generation');
        }
        ...
        // ✅ Add ONLY Abaqus commands for each test case
        for (const test of testData) {
            ...
            if (!jobName) {
                console.warn(`⚠️ Skipping test #${test.number_of_runs}: No job name`);
                skippedCount++;
                continue;
            }
            ...
            if (!fs.existsSync(folderPath)) {
                console.warn(`⚠️ Skipping test #${test.number_of_runs}: Folder not found: ${folderPath}`);
                skippedCount++;
                continue;
            }
            
            // ✅ VERIFY .inp FILE EXISTS
            const inpFilePath = path.join(folderPath, `${jobName}.inp`);
            if (!fs.existsSync(inpFilePath)) {
                console.warn(`⚠️ Skipping test #${test.number_of_runs}: .inp file not found: ${inpFilePath}`);
                skippedCount++;
                continue;
            }
            
            // ✅ Change directory to test folder (CRITICAL)
            batchContent += `cd /d "${folderPath}"\n`;
            
            // ✅ BUILD ABAQUS COMMAND WITH CORRECT PARAMETER ORDER
            // Order: job → oldjob → input → cpus → interactive
            let abaqusCmd = `call abaqus job=${jobName}`;
            
            // ✅ ADD oldjob PARAMETER IF SPECIFIED (NO .odb CHECK)
            if (oldJobName && oldJobName.trim() !== '' && oldJobName !== '-') {
                abaqusCmd += ` oldjob=${oldJobName}`;
                // ✅ REMOVED: No console logging for oldjob
            }
            
            // ✅ ADD input, cpus, AND interactive PARAMETERS
            abaqusCmd += ` input=${jobName}.inp cpus=${ABAQUS_CPU_COUNT} interactive`;
            
            batchContent += `${abaqusCmd}\n`;
            commandCount++;
            
            // ✅ For CDTire: Add Fortran and Python commands if present
            if (protocol === 'CDTire') {
                const fortranFile = test.fortran_file || '';
                const pythonScript = test.python_script || '';
                
                // ✅ Fortran job (if specified)
                if (fortranFile && fortranFile.trim() !== '' && fortranFile !== '-') {
                    // Verify Fortran file exists
                    const fortranPath = path.join(folderPath, fortranFile);
                    if (fs.existsSync(fortranPath)) {
                        // ✅ FORTRAN COMMAND ALSO USES CORRECT ORDER: job → user → oldjob → cpus → interactive
                        batchContent += `call abaqus job=${jobName}_fortran user=${fortranFile} oldjob=${jobName} cpus=${ABAQUS_CPU_COUNT} interactive\n`;
                        commandCount++;
                    }
                }
                
                // ✅ Python script (if specified)
                if (pythonScript && pythonScript.trim() !== '' && pythonScript !== '-') {
                    // Verify Python file exists
                    const pythonPath = path.join(folderPath, pythonScript);
                    if (fs.existsSync(pythonPath)) {
                        batchContent += `call abaqus python ${pythonScript}\n`;
                        commandCount++;
                    }
                }
            }
 ...
 * ✅ FIXED: Correctly resolves paths to P_L subfolders
...
async function runPythonScript(folderPath, pythonFileName, jobName, folderName, logger = null) {
    return new Promise((resolve, reject) => {
        if (!pythonFileName || pythonFileName.trim() === '' || pythonFileName === '-') {
            console.log(`⚠️ No Python script specified for ${jobName}, skipping`);
            return resolve({ success: true, skipped: true });
        }

        // ✅ CRITICAL FIX: Python script should be in the SAME folder as .inp files (P_L subfolder)
        const pythonPath = path.join(folderPath, pythonFileName);
        
        console.log(`\n🐍 [Python Script Execution]`);
        console.log(`   Job: ${jobName}`);
        console.log(`   Script: ${pythonFileName}`);
        console.log(`   Folder: ${folderName}`);
        console.log(`   Full path: ${pythonPath}`);

        // ✅ VERIFY FILE EXISTS BEFORE RUNNING
        if (!fs.existsSync(pythonPath)) {
            const errorMsg = `Python script not found: ${pythonPath}`;
            console.error(`   ❌ ${errorMsg}`);
            
            // ✅ CHECK IF FILE EXISTS IN TEMPLATE (for debugging)
            const templatePath = path.join(__dirname, 'templates', 'CDTire', pythonFileName);
            if (fs.existsSync(templatePath)) {
                console.error(`   ⚠️ File exists in templates but wasn't copied to project folder`);
        ...
        console.log(`   ✅ Python script file verified`);

        // ✅ CONSTRUCT ABAQUS PYTHON COMMAND
        // Run in the P_L folder (folderPath) where the script is located
        const pythonCmd = `call abaqus python ${pythonFileName}`;
        ...
        if (logger) {
            ...
            logger.info(`Script: ${pythonFileName}`);
            logger.info(`Folder: ${folderName}`);
            logger.info(`Command: ${pythonCmd}`);
        }

        // ✅ EXECUTE PYTHON SCRIPT IN THE CORRECT FOLDER
        const pythonProcess = spawn('cmd.exe', ['/c', pythonCmd], {
            cwd: folderPath, // ✅ Run in P_L folder where script is located
            stdio: ['ignore', 'pipe', 'pipe'],
            windowsHide: true
        });
        ...
        pythonProcess.on('close', (code) => {
            ...
            if (logger) {
                logger.info(`Python script exit code: ${code}`);
                if (code === 0) {
                    logger.success('Python script completed successfully');
                } else {
        ...
        pythonProcess.on('error', (error) => {
            const errorMsg = `Failed to start Python script: ${error.message}`;
            console.error(`   ❌ ${errorMsg}`);
            if (logger) logger.error(errorMsg);
